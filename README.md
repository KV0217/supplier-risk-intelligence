...
**Composite Score (Current ML Path):**
```
V1 fallback: Final Risk = (News Risk × 0.60) + (Financial Risk × 0.40)

V2 ML mode (implemented): XGBoost regressor learns supplier risk from engineered
news + financial features. When historical disruption labels are unavailable, the
system uses weak supervision by bootstrapping labels from rule-based scores, then
switches to learned inference.
...
- [x] ML model training (XGBoost, weak supervision bootstrap)
