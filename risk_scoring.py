...
class RiskScoringEngine:
    ...
    def _train_ml_weak_supervision(self, features_df: pd.DataFrame) -> bool:
        """
        Train multiple ML models on weak labels generated by the current rule-based scorer.
        Automatically evaluates and selects the best performing model.
        """
        ...
        models = {
            ...
            "SVR": make_pipeline(StandardScaler(), SVR(C=100.0, epsilon=0.1))
        }
        
        try:
            from xgboost import XGBRegressor
            models["XGBoost"] = XGBRegressor(n_estimators=100, learning_rate=0.05, max_depth=4, random_state=42, objective="reg:squarederror")
        except ImportError:
            logger.info("xgboost not installed, skipping XGBoost in model evaluation.")
    ...
    def score_suppliers(
        ...
        if not self.ml_available:
            self.ml_available = self._train_ml_weak_supervision(features_df)
        ...
        for _, row in features_df.iterrows():
            ...
            # Always calculate rule-based baseline to act as an anchor/fallback
            rule_score, _ = self.calculate_composite_risk_score_rule_based(
                news_risk, financial_risk, article_count
            )

            if self.ml_available and self.ml_model is not None:
                try:
                    ml_score = self._predict_with_ml(row.to_dict())
                    
                    # Extract the actual model name, handling sklearn Pipelines
                    actual_model = self.ml_model.steps[-1][1] if hasattr(self.ml_model, 'steps') else self.ml_model
                    model_name = type(actual_model).__name__.replace('Regressor', '')
                    if model_name == 'XGB': model_name = 'XGBoost'
                    # Ensemble: Blend 70% ML and 30% Rule-Based to anchor predictions
                    composite_score = (ml_score * 0.7) + (rule_score * 0.3)
                    risk_level = self._risk_level_from_score(composite_score)
                    model_used = f"Ensemble (70% {model_name}, 30% Rules)"
                except Exception as e:
                    logger.warning(f"ML prediction failed for {row['company']}: {e}")
